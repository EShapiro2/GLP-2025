...
\verb|now(Integer).| & Binds argument to current Unix timestamp (milliseconds) \\
\end{tabular}
\end{center}
...
Arithmetic assignment is defined by the following clause:
\begin{verbatim}
X? := E :- ground(E?) | true.
\end{verbatim}
The guard \verb|ground(E?)| ensures the expression is fully evaluated before the assignment is made. The runtime evaluates the arithmetic expression and binds the result to \verb|X|.
