...
GLP terms, unit goals, goals, and clauses are as in standard LP (Appendix~\ref{appendix:lp}) but defined over the variables in $\hat\calV$.
...
Despite these differences, GLP adopts the same notions of proper run, successful run, and outcome as LP (Definition~\ref{def:proper-run}, Appendix~\ref{appendix:lp}), and has the same notion of logical consequence as LP. Let $/?$ be an operator that replaces every reader by its paired writer.

\begin{restatable}[GLP Computation is Deduction]{proposition}{propGLPDeduction}
\label{prop:glp-deduction}
Let $(G_0$:- $G_n)\sigma$ be the outcome of a proper GLP run $\rho: (G_0,\sigma_0) \rightarrow \cdots \rightarrow (G_n, \sigma_n)$ of $GLP(P)$. Then $(G_0$ :- $G_n)\sigma/?$ is a logical consequence of $P/?$.
\end{restatable}
