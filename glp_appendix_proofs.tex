...
For the inductive step, assume $G_i$ satisfies SO and consider $G_i \rightarrow G_{i+1}$ via reduction of atom $A \in G_i$ with clause $C$ satisfying the SRSW syntactic restriction. Let $(H \verb|:-| B)$ be the renaming of $C$ apart from $G_i$, with writer mgu $\sigma$.

Since the clause satisfies SRSW, it satisfies SO: every variable in $C$ occurs exactly once. After renaming apart, $(H \verb|:-| B)$ also satisfies SO with fresh variables.
