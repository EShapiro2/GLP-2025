...
GLP terms, unit goals, goals, and clauses are as in LP (Definition~\ref{def:lp-syntax}) but defined over the variables in $\hat\calV$. Let $\calG_?(P)$ denote the set of goals over $\hat\calV$ restricted to the vocabulary of $P$.
...
The \temph{anonymous variable} \verb|_| in GLP programs may appear anywhere a writer variable may appear. Each occurrence denotes a fresh writer with no paired reader, providing a controlled exception to the SRSW restriction. Values assigned to \verb|_| are discarded. For example:
\begin{verbatim}
second([_, X | _], X?).
foo(X) :- bar(_, X?).
\end{verbatim}
...
Two GLP terms \temph{have a variable in common} if for some writer $X \in \calV$, either $X$ or $X?$ occurs in both. A GLP renaming $\sigma$ \temph{renames $T'$ apart from} $T$ if $T'\sigma$ and $T$ have no variable in common.
\end{definition}
...
The GLP Communicate rule realises the use of reader/writer pairs for asynchronous communication: It applies an assignment to a reader after it has been applied to its paired writer.
...
Despite these differences, GLP can adopt the same notion of successful run and outcome of LP (Definition~\ref{def:proper-run}), and have the same notion of logic consequence as LP. Let $/?$ be an operator that replaces every reader by its paired writer.
...
A \temph{guarded clause} has the form $H$ \verb|:-| $G$ \verb"|" $B$, where $H$ is the head, $G$ is a conjunction of guard predicates, and $B$ is the body. The guard separator ``\verb"|"'' distinguishes guards from the body. A clause without guards is written $H$ \verb|:-| $B$, or equivalently $H$ \verb|:-| \verb|true| \verb"|" $B$.
\end{definition}
...
Guard predicates include type tests (\verb|integer|, \verb|number|, \verb|string|, \verb|constant|, \verb|compound|, \verb|ground|, \verb|known|), arithmetic comparisons (\verb|<|, \verb|>|, \verb|=<|, \verb|>=|, \verb|=:=|, \verb|=\=|), ground equality (\verb|=?=|), and the control guard \verb|otherwise|.
...
Here we prove that, like LP, GLP computations are deductions, and that a goal that can be reduced in a configuration can still be reduced in any subsequent configuration of the computation.
