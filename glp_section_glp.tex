...
GLP clauses may include \emph{guards}---pure tests that determine clause applicability without participating in data flow.
...
Guards are evaluated before clause commitment.  Each guard has three-valued semantics: it may \emph{succeed}, \emph{fail}, or \emph{suspend} (when a required reader argument is unbound).  A clause is applicable only if all its guards succeed.  If any guard fails, the clause is skipped and the next clause is tried.  If a guard suspends, clause selection suspends pending further instantiation.

\begin{example}[Guards]
The following clause uses a guard to test groundness:
\begin{verbatim}
broadcast(X, Y1, Y2) :- ground(X?) | send(X?, Y1), send(X?, Y2).
\end{verbatim}
The guard \verb|ground(X?)| succeeds if \verb|X| is bound to a ground term, and suspends if \verb|X| is unbound.  Once the guard succeeds, the body may safely contain multiple occurrences of the reader \verb|X?|.
\end{example}

\begin{remark}[Guards and SRSW]
\label{rem:guards-srsw}
Guard occurrences do not count toward SRSW satisfaction.  For SRSW purposes, a variable occurs in a clause $C = (H$ \verb|:-| $G$ \verb"|" $B)$ if it occurs in $H$ or $B$; occurrences in $G$ are disregarded.  This reflects the semantic role of guards as tests rather than consumers: a guard examines a value but does not constitute the ``consumption'' that pairs with production.

The \verb|ground(X)| guard relaxes the single-reader constraint for \verb|X?| within that clause's body, since multiple occurrences of a reader instantiated to a ground term cannot violate the single-writer requirement.
\end{remark}

Appendix~\ref{appendix:guards-system} provides a complete specification of guard predicates, including type guards (\verb|ground|, \verb|known|, \verb|writer|, \verb|reader|), comparison guards (\verb|=|, \verb|=\=|), and the control guard \verb|otherwise|, as well as system predicates for arithmetic evaluation and runtime services.
